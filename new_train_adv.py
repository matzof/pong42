...
from new_model_adv import Policy
...
for it in range(100000):
    ...
    for _ in range(5):
        ...
        reward_batch = torch.FloatTensor([discounted_rewards[idx] for idx in idxs])
        #advantage_batch = (advantage_batch - advantage_batch.mean()) / advantage_batch.std()
              
        opt.zero_grad()
        loss = policy.PPO_update(d_obs_batch, action_batch, action_prob_batch, reward_batch)
        loss.backward()
        opt.step()

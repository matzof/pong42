...
for ep in range(episodes):
    ...
    epsilon = glie_a/(glie_a+ep*2)
    length_ep = 0
    while not done:
        # Get the actions from both SimpleAIs
        action1 = player.get_action(ob1, epsilon, model)
        action2 = opponent.get_action(ob2)
        # Step the environment and get the rewards and new observations
        previous_state = ob1
        (ob1, ob2), (rew1, rew2), done, info = env.step((action1, action2))
        
        player.agent.store_transition(previous_state, action1, ob1, rew1, done, model)
        length_ep += 1
        # Count the wins
        if rew1 == 10:
            win1 += 1
